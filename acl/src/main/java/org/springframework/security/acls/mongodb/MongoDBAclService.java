 ...
 * <p>
 * This implementation differs from the SQL based implementation by having a single
 * MongoDB collection containing all the necessary ACL related data per document in a
 * non-final structure represented by the {@link MongoAcl} POJO. This service will convert
 * database results from POJO to ACL related classes like {@link Acl},
 * {@link ObjectIdentity}, {@link Sid} and {@link AccessControlEntry} instances
 * internally.
 *
 * @author Ben Alex
...
public class MongoDBAclService implements AclService {
	...
	public List<ObjectIdentity> findChildren(ObjectIdentity parentIdentity) {
		...
		List<MongoAcl> aclsForDomainObject = this.aclRepository
				.findByInstanceIdAndClassName(parentIdentity.getIdentifier(), parentIdentity.getType());
		...
		for (MongoAcl acl : aclsForDomainObject) {
			List<MongoAcl> childAclsOfDomainObject = this.aclRepository.findByParentId(acl.getId());
		...
		for (MongoAcl child : children) {
			try {
				...
				LOG.error("Could not find class of domain object '{}' referenced by ACL {}", child.getClassName(),
						child.getId());
			}
		}
