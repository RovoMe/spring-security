 ...
 * <p>
 * This implementation will map ACL related classes like {@link Acl},
 * {@link AccessControlEntry} and {@link Sid} to a {@link MongoAcl} POJO class which is
 * persisted or accessed via a MongoDB based aclRepository. This POJO will contain all the
 * ACL relevant data for a domain object in a non flat structure. Due to the non-flat
 * structure lookups and updates are relatively trivial compared to the SQL based
 * {@link AclService} implementation.
 *
 * @author Ben Alex
...
public class MongoDBMutableAclService extends MongoDBAclService implements MutableAclService {
	...
	public MutableAcl createAcl(ObjectIdentity objectIdentity) throws AlreadyExistsException {
		...
		List<MongoAcl> availableAcl = aclRepository.findByInstanceIdAndClassName(objectIdentity.getIdentifier(),
		...
		MongoAcl mongoAcl = new MongoAcl(objectIdentity.getIdentifier(), objectIdentity.getType(),
				UUID.randomUUID().toString(), new MongoSid(sid.getPrincipal()), null, true);
	...
	public MutableAcl updateAcl(MutableAcl acl) throws NotFoundException {
		...
		for (AccessControlEntry _ace : acl.getEntries()) {
			...
			DomainObjectPermission permission = new DomainObjectPermission(aceId, sid, ace.getPermission().getMask(),
					ace.isGranting(), ace.isAuditSuccess(), ace.isAuditFailure());
			mongoAcl.getPermissions().add(permission);
		}

		// Update the acl entry
		this.aclRepository.save(mongoAcl);
