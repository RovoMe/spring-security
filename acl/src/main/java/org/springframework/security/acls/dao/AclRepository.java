...
public interface AclRepository extends MongoRepository<MongoAcl, Serializable> {
	 ...
	 */
	@NonNull
	Optional<MongoAcl> findById(@NonNull Serializable id);

	/**
	 * Returns the ACL for a given domain object identifier and its class name.
	 * @param instanceId The unique identifier of the domain object the ACL should be
	 * returned for
	...
	/**
	 * Retrieves all child ACLs which specified the given <em>parentId</em> as their
	 * parent.
	...
	/**
	 * Removes a document from the ACL collection that contains an instanceId field set to
	 * the provided value.
	 * @param instanceId The unique identifier of the domain object to remove an ACL entry
	 * for
